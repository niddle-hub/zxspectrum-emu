...
static const int red[16] = { 0, 0, 168, 160, 0, 0, 168, 168,
...
_width(width), _height(height){
	...
	if(_window == nullptr)
		throw std::runtime_error(
				std::string("Ошибка создания окна: ") +
				SDL_GetError());
	...
	if(_renderer == nullptr)
		throw std::runtime_error(
				std::string("Ошбибка создания рендера: ")+
				SDL_GetError());
}
...
void Window::render() {
	...
	for(unsigned row = 0; row < 192; ++row){
		for (unsigned col = 0; col <32; ++col){
			...
			unsigned a10a8 = (point >> 8) & 0x007;
			unsigned a7a5 = (point >> 5) & 0x007;
			unsigned a12a11 = (point >> 11) & 0x003;
			...
			unsigned attr_addr =
					...
					uint8_t paper_col = ((attr >> 3) & 0x07) | ((attr & 0x04) >> 3);
					uint8_t ink_col = (attr & 0x07) | ((attr & 0x04) >> 3);
